...
import avalaunch_logo from './../images/avalaunch_logo.png'

const Header = props => {
  const { Moralis, user } = useMoralis()
  const [chainId, setChainId] = useState('')
  const [avaxBal, setAvaxBal] = useState('')

  const displayAddress = address => {
    return address
      .slice(0, 5)
      .concat('...')
      .concat(address.slice(address.length - 5, address.length))
  }
  ...
  useEffect(() => {
    async function getChainId () {
      try {
        ...
        setChainId(web3Provider.provider.chainId)
  ...
  return (
    <>
      <div className='flex w-full border-b border-t border-black shadow-lg pl-3 py-2 -mt-8 fixed bg-white z-10'>
        <svg
          width='60'
          height='38'
          viewBox='0 0 50 38'
        ...
        <p
          className='text-gray-800 text-2xl ml-3'
          style={{ fontFamily: 'IntegralCF-ExtraBold' }}
        >
          MORALIS HACKATHON 2022
        </p>
        <img className='h-9 ml-5' src={avalaunch_logo} alt='avalaunch logo' />
        <p
          className='text-gray-800 text-base ml-3 my-auto'
          style={{ fontFamily: 'IntegralCF-ExtraBold' }}
        >
          (Avalaunch Stream)
        </p>
        {/* <h1
        className={`mx-auto mt-2 text-3xl font-bold ${
          Moralis.getChainId() === '0xa869' ? 'text-lime-500' : 'text-red-500'
        }`}
      >
        {Moralis.getChainId()}
      </h1> */}
        {Moralis.getChainId() === '0xa869' ? (
          <h1 className='flex ml-auto font-bold text-lime-500 text-lg'>
            <span className='my-auto text-3xl'>🗹</span>
            <span className='ml-1 my-auto'>Connected to:</span>
          </h1>
        ) : (
          <h1 className='my-auto ml-auto font-bold text-red-500 text-lg animate-pulse'>
            <span className='my-auto text-xl'>❌</span>
            <span className='ml-1 my-auto'>Please connect to:</span>
          </h1>
        )}
        <h1 className='text-lg font-bold my-auto ml-3 border-2 shadow-lg border-gray-500 px-2 rounded-2xl'>
          Avalanche Testnet
        </h1>
        <h1 className='text-stone-400 ml-3 mr-3 my-auto font-mono text-base border border-gray-700 p-1 rounded-xl'>
          {/* {props.title} */}
          {/* {user.attributes.ethAddress} */}
          {displayAddress(user.attributes.ethAddress)}
          {/* {console.log(user)} */}
          {/* {chainId} */}
        </h1>
      </div>
